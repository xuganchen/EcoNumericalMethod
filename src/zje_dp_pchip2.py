...
def zje_dp_derivative(x,
                      alpha, beta, delta,
                      kgrid,
                      pp2, pp3):
  c = np.power(kgrid, alpha) + (1 - delta) * kgrid - x
  y = - np.power(c, -1) + beta * pp2(x)
  return y


def zje_dp_derivative_jac(x,
                          alpha, beta, delta,
                          kgrid,
                          pp2, pp3):
  c = np.power(kgrid, alpha) + (1 - delta) * kgrid - x
  yp = np.diag(- np.power(c, -2) + beta * pp3(x))
  return yp


def zje_dp_pchip2(alpha, beta, delta,
                  m_kgrid, m_kgrid2,
                  iternum,
                  figurepath=None):
  '''
  ...
  for kkk in range(iternum):
    ...
    x0 = optk
    res = spopt.least_squares(zje_dp_derivative, x0,
                              jac=zje_dp_derivative_jac,
                              # hess=zje_dp_derivative_hess,
                              method="dogbox",
                              bounds=(lb, ub),
                              args=(alpha, beta, delta, kgrid, pp2, pp3,))
    optk2 = res.x
  ...
  title = "zje_dp_pchip2-eulereuqtionerrors.png"
...
if __name__ == "__main__":
  ...
  figurepath = "../figure"
  zje_dp_pchip2(alpha, beta, delta,
                m_kgrid, m_kgrid2,
                iternum,
